...
GraphAPI.prototype.batchRequest = function(batch, callback) {
    //batch must be a JSON object of the form [{method: GET, relative_url: '/asdf'}, {method: POST, relative_url: '/asdf'}]
    return this.request('/', 'POST', null, {batch:JSON.stringify(batch)}, callback);
};
...
/*--------------------------*/
   ...
   * to not be available.
   *
   * @return object A valid user access token, user id, and unix timestamp of issue, or false if one
   *                could not be determined; 
   */
exports.getUser = function (cookies, appId, appSecret, callback) {
    var signed_request = this.getSignedRequest(cookies, appId, appSecret);
    if(signed_request) {
        if(signed_request.oauth_token) {
            callback(null, {access_token:signed_request.oauth_token, issued_at:signed_request.issued_at, user_id:signed_request.user_id});
            ...
            this.getAccessTokenFromCode(appId, appSecret, signed_request.code, function(error, accessToken) {
                if(error) { callback(error, false); }
                    ...
                    callback(null, {access_token:accessToken, issued_at:signed_request.issued_at, user_id:signed_request.user_id});
                }
            });
        } else {
            callback(null, false);
