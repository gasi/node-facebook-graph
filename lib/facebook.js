...
exports.decodeSignedRequest = function (signedRequest, appSecret) {
    ...
    var signature = decodeBase64URL(parts[0]);
    // var encodedSignature = parts[0];
    var encodedPayload = parts[1];

    var data = JSON.parse(decodeBase64URL(encodedPayload));
    ...
    // var binaryDigest = hmac.digest('binary');
    var base64Digest = hmac.digest('base64');
    var base64URLDigest = base64Digest.replace(/\+/g, '-')
                                      .replace(/\//g, '_')
                                      //.replace(/[=]{1,3}/g, ''); // remove padding
    // var expectedSignature = binaryDigest;
...
function decodeBase64URL(input) {
    var base64Input = input.replace(/-/g, '+').replace(/_/g, '/');
    // var numPads = (4 - base64Input.length % 4) % 4;
    // for (var i = 0; i < numPads; i++) {
    //     base64Input += '=';
    // }
    var buffer = new Buffer(base64Input, 'base64');
